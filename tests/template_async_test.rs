...
fn test_async_template_generation() {
    ...
    rt.block_on(async {
        ...
        for file in required_files {
            ...
            assert!(file_path.exists(), "File does not exist: {}", file_path.display());
        }
    ...
    rt.block_on(async {
        ...
        for i in 0..5 {
            ...
            handles.push(tokio::spawn(async move {
                engine.generate_async("basic", context, &output_dir).await.unwrap();
            }));
        }
        ...
        for i in 0..5 {
            ...
            for file in required_files {
                let file_path = output_dir.join(file);
                assert!(file_path.exists(), "File does not exist in project {}: {}", i, file_path.display());
            }
        }
    });
}
...
fn test_async_template_error_handling() {
    ...
    rt.block_on(async {
        ...
        let result = engine.generate_async("non_existent_template", context, &output_dir).await;
...
fn test_async_template_variable_substitution() {
    ...
    rt.block_on(async {
        ...
        assert!(dockerfile.contains("org.opencontainers.image.authors=\"testuser <contact@example.com>\""));
