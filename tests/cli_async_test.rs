...
async fn test_cli_async_template_generation() {
    ...
    let entrypoint = std::path::Path::new("test/project/runtime-assets/usr/local/bin/entrypoint.sh");
...
async fn test_cli_async_template_with_options() {
    ...
    assert!(dockerfile_content.contains("org.opencontainers.image.authors=\"customuser <contact@example.com>\""));
